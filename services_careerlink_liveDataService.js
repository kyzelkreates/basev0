 ...
 *
 * Internal function names (fetchJobseekers, upsertJobseeker etc.)
 * preserved for Supabase column name compatibility.
 * Use participant* aliases for all new code.
 * ============================================================
 */
...
/**
 * Fetch all participants for a coach.
...
export async function fetchJobseekers({ organisationId } = {}) {
  ...
  try {
    let q = sb().from('participants').select('*').order('created_at', { ascending: false })
...
/**
 * Generate a deterministic public link ID from participant ID.
...
// ─── Participant Aliases (preferred — use these in new code) ──
// Legacy jobseeker function names preserved above for Supabase column compatibility
export { fetchJobseekers as fetchParticipants }
export { upsertJobseeker as upsertParticipant }
export { resolveJobseekerFromLink as resolveParticipantFromLink }
export { normaliseJobseeker as normaliseParticipant }
