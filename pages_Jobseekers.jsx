    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-lg bg-[#0d1426] border border-slate-800/60 rounded-2xl overflow-hidden animate-slide-up">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800/40">
          <span className="font-semibold text-white">{existing?.id ? 'Edit Participant' : 'New Participant'}</span>
...
function JobseekerDetail({ participant, onBack, onEdit, weeklyTarget, dataStore, isDemoMode }) {
  ...
  const recentApps = dataSnap.applications.filter(a => a.jobseekerId === participant.id).slice(0, 5)
  const recentIvs  = dataSnap.interviews.filter(iv => iv.jobseekerId === participant.id).slice(0, 3)
  const recentCIs  = dataSnap.checkIns.filter(ci => ci.jobseekerId === participant.id).slice(0, 5)
  const evidence   = (isDemoMode ? dataStore.evidenceRecords : dataStore.evidenceRecords.filter(r => !r.isDemo))
    .filter(e => e.jobseekerId === participant.id).slice(0, 5)
  const notes = (isDemoMode ? dataStore.coachNotes : dataStore.coachNotes.filter(r => !r.isDemo))
    .filter(n => n.jobseekerId === participant.id)
  const flags = (isDemoMode ? dataStore.supportFlags : dataStore.supportFlags.filter(r => !r.isDemo))
    .filter(f => f.jobseekerId === participant.id)

  const addNote = () => {
    if (!noteText.trim()) return
    dataStore.addCoachNote({ jobseekerId: participant.id, note: noteText, createdBy: 'coach' })
    setNoteText('')
  }
  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="font-display font-bold text-white text-lg">{participant.displayName}</h2>
          <p className="text-slate-500 text-xs">{participant.programme || 'No programme set'}</p>
        </div>
        {participant.isDemo && <span className="text-[10px] text-amber-500/60 font-mono ml-2">[DEMO]</span>}
      ...
      <div className="bg-[#0d1426] border border-slate-800/60 rounded-xl p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-xs text-slate-500">{metrics.weeklyHoursLogged}h / {participant.weeklyTargetHours || weeklyTarget}h</span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-[#0d1426] border border-slate-800/60 rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-800/40 flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-white">Activity Records</span>
          ...
          <div className="divide-y divide-slate-800/20">
            {recentApps.length === 0 ? (
              ...
              <div key={a.id} className="px-4 py-2.5">
                ...
                <div className="text-[10px] text-slate-600">{a.organisation}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-[#0d1426] border border-slate-800/60 rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-800/40 flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-white">Mentor Notes</span>
          </div>
...
export default function Participants() {
  ...
  const filtered = participants.filter(j =>
  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="font-display font-bold text-white text-xl">Participants</h1>
          <p className="text-slate-500 text-sm mt-0.5">{participants.length} total · weekly target: {weeklyTarget}h</p>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-800/60">
                  {['Name','Status','Weekly Hours','Target %','Activity Records','Risk','Last Check-in'].map(h => (
