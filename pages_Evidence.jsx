...
const EVIDENCE_TYPES = [
  'Activity Record confirmation','Review Session confirmation','Training certificate',
  'Progress portfolio / Cover letter','Organisation correspondence','Activity tracking screenshot',
  'Activity log export','Attendance record','Other',
]

function EvidenceModal({ existing, participants, onClose, onSaved }) {
  const dataStore = useDataStore()
  const [form, setForm] = useState({
    jobseekerId:'', title:'', evidenceType:'Activity Record confirmation',
        ...
        <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <div>
            ...
            <input value={form.title} onChange={e => set('title', e.target.value)}
              placeholder="e.g. Horizon Ltd Activity Record Confirmation"
...
export default function Evidence() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-800/60">
                  {['Participant','Title','Type','Date','File Reference','Notes'].map(h => (
      ...
      {modal !== null && <EvidenceModal participants={participants} existing={modal} onClose={() => setModal(null)} onSaved={() => {}}/>}
    </div>
  )
}
