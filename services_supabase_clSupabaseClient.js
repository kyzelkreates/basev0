...
/**
 * Get (or create) the 4P3X Base Supabase client.
...
export function getCLSupabaseClient() {
  ...
  if (env.warnings.length > 0) {
    env.warnings.forEach(w => console.warn('[4P3X Base:Supabase]', w))
  }
  ...
  try {
    ...
    console.info('[4P3X Base:Supabase] ✓ Client ready', { url: maskSecret(env.url, 30) })
    return _client
  } catch (e) {
    _status = 'error'
    console.error('[4P3X Base:Supabase] Client creation failed:', e.message)
    return null
  }
}
