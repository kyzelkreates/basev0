...
function ReportView({ participant, dataStore, isDemoMode, weeklyTarget }) {
  ...
  const logs     = snap.activityLogs.filter(r=>r.jobseekerId===participant.id)
  const apps     = snap.applications.filter(r=>r.jobseekerId===participant.id)
  const ivs      = snap.interviews.filter(r=>r.jobseekerId===participant.id)
  const cis      = snap.checkIns.filter(r=>r.jobseekerId===participant.id)
  const evidence = (isDemoMode ? dataStore.evidenceRecords : dataStore.evidenceRecords.filter(r=>!r.isDemo)).filter(r=>r.jobseekerId===participant.id)
  const flags    = (isDemoMode ? dataStore.supportFlags    : dataStore.supportFlags.filter(r=>!r.isDemo)).filter(r=>r.jobseekerId===participant.id)
  const notes    = (isDemoMode ? dataStore.coachNotes      : dataStore.coachNotes.filter(r=>!r.isDemo)).filter(r=>r.jobseekerId===participant.id)
  const tasks    = (isDemoMode ? dataStore.tasks           : dataStore.tasks.filter(r=>!r.isDemo)).filter(r=>r.jobseekerId===participant.id)
  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="font-display font-bold text-white text-xl">{participant.displayName}</h2>
        ...
        {participant.isDemo && <span className="text-[10px] text-amber-500/60 font-mono">[DEMO]</span>}
      </div>
      ...
      <Section title="Weekly Activity Summary" icon="BarChart3">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            ['Activity Records', metrics.applicationCountWeek,'text-purple-400'],
            ['Upcoming Review Sessions', metrics.interviewsUpcoming,'text-cyan-400'],
          ].map(([l,v,c]) => (
        ...
        <div className="mt-3">
          <div className="flex items-center justify-between mb-1">
            <span className="text-xs text-slate-500">Progress to {participant.weeklyTargetHours||weeklyTarget}h weekly target</span>
          ...
          <div className="space-y-2">
            {apps.map(a => (
              <div key={a.id} className="flex items-center gap-3 border-b border-slate-800/20 pb-2 last:border-0">
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-medium text-white">{a.roleTitle} at {a.organisation}</div>
      ...
      {/* Mentor Notes */}
      {notes.length > 0 && (
        <Section title={`Mentor Notes (${notes.length})`} icon="StickyNote">
          {notes.map(n => (
...
export default function Reports() {
  ...
  if (selected) {
    return <ReportView participant={selected} dataStore={dataStore} isDemoMode={isDemoMode} weeklyTarget={weeklyTarget}/>
  }
  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-0.5">Select a participant to view their activity report.</p>
      </div>
      {participants.length === 0 ? (
        <div className="py-12 text-center bg-[#0d1426] border border-slate-800/60 rounded-xl">
          ...
          <p className="text-sm text-slate-600">No participants found.</p>
        </div>
      ) : (
