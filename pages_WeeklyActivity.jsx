...
function ActivityModal({ existing, participants, onClose, onSaved }) {
  const [form, setForm] = useState({
    jobseekerId: '', date: new Date().toISOString().split('T')[0],
    startTime: '', endTime: '', durationMinutes: 60,
    activityType: 'Activity tracking', description: '', evidenceAttached: false,
    relatedActivityRecord: false, notes: '',
  })
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-lg bg-[#0d1426] border border-slate-800/60 rounded-2xl overflow-hidden animate-slide-up">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800/40">
          <span className="font-semibold text-white">{existing?.id ? 'Edit Activity' : 'Log Activity Tracking Activity'}</span>
        ...
        <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <div className="flex gap-4">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.relatedActivityRecord} onChange={e => set('relatedActivityRecord', e.target.checked)} className="accent-[#d4af37]" />
              <span className="text-xs text-slate-400">Related to activity record</span>
            </label>
          </div>
          <div>
...
export default function WeeklyActivity() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-800/60">
                  {['Participant','Date','Activity Type','Duration','Description','Evidence'].map(h => (
      ...
      {modal !== null && (
        <ActivityModal participants={participants} existing={modal} onClose={() => setModal(null)} onSaved={() => {}} />
      )}
    </div>
  )
}
