...
const APPLICATION_STATUSES = ['Drafting','Applied','Awaiting response','Review Session offered','Review Session completed','Rejected','Offer received','Withdrawn']
const BARRIER_TYPES = [
        ...
        <div className="bg-[#0d1426] border border-slate-800/60 rounded-2xl p-6 space-y-4">
          <div>
            ...
            <select value={jsId} onChange={e => setJsId(e.target.value)}
              ...
              {participants.map(js => <option key={js.id} value={js.id}>{js.displayName}</option>)}
            </select>
          </div>
          ...
          {participants.length === 0 && (
            <p className="text-[10px] text-slate-600 text-center leading-relaxed">
              No participants set up yet. Ask your employment coach to set up your account.
            </p>
          )}
        </div>
      </div>
...
// ─── Participant AI Support Assistant ─────────────────────────
// ─── 4P3X Intelligent AI 3 + AI 4 (PWA — separated) ─────────
function PWAAssistants({ participant, metrics, weeklyTarget, dataStore }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {/* AI 4 — Support Assistant & Encouragement */}
      <div>
        <div className="flex items-center gap-2 mb-2">
...
function HomeScreen({ participant, metrics, weeklyTarget, onQuickLog, navigate, dataStore }) {
  ...
  return (
    <div className="p-4 space-y-5">
      <div>
        <h2 className="text-base font-semibold text-white">Hi, {participant.displayName.split(' ')[0]} 👋</h2>
        ...
        <div className="flex items-center justify-around gap-4">
          ...
          <div className="space-y-3 flex-1">
            <div className="flex justify-between">
              ...
              <span className="text-xs font-mono text-white">{metrics.weeklyHoursLogged}h / {participant.weeklyTargetHours || weeklyTarget}h</span>
            </div>
            <div className="flex justify-between">
              <span className="text-xs text-slate-400">Activity Records</span>
              <span className="text-xs font-mono text-white">{metrics.applicationCountWeek}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-xs text-slate-400">Review Sessions</span>
      ...
      <PWAAssistants participant={participant} metrics={metrics} weeklyTarget={weeklyTarget} dataStore={dataStore}/>
    </div>
  )
}

function LogActivityScreen({ participant, onBack, onSaved }) {
  const dataStore = useDataStore()
  const [form, setForm] = useState({
    date: new Date().toISOString().split('T')[0],
    startTime:'', endTime:'', durationMinutes:60,
    activityType:'Activity tracking', description:'', evidenceAttached:false, notes:''
  })
  ...
  const handleSave = () => {
    ...
    setSaving(true)
    dataStore.addActivityLog({ ...form, jobseekerId: participant.id })
    setSaving(false)
    setSuccess(true)
  ...
  if (success) return (
    <div className="p-6 flex flex-col items-center justify-center min-h-[60vh] text-center">
      ...
      <p className="text-slate-500 text-sm mt-1">Your activity tracking time has been recorded.</p>
    </div>
  )
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="flex items-center gap-3">
        ...
        <h2 className="font-semibold text-white">Log Activity Tracking Activity</h2>
      </div>
...
function CheckInScreen({ participant, onBack }) {
  const dataStore = useDataStore()
  ...
  const handleSubmit = () => {
    ...
    if (supportNeeded) {
      barriers.forEach(b => {
        dataStore.addSupportFlag({ jobseekerId: participant.id, type: b,
          severity: confidence < 4 ? 'high' : 'medium',
          description: `Flagged by participant via daily check-in. Confidence: ${confidence}/10.`, status: 'open' })
      })
    }
    setSubmitted(true)
  }
...
function SupportScreen({ participant, onBack }) {
  const dataStore = useDataStore()
  ...
  const handleSubmit = () => {
    ...
    selected.forEach(type => {
      dataStore.addSupportFlag({ jobseekerId: participant.id, type, severity: 'medium',
        description: message || 'Support requested by participant via PWA.', status: 'open' })
    })
    if (selected.length === 0 && message.trim()) {
      dataStore.addSupportFlag({ jobseekerId: participant.id, type: 'other_barrier', severity: 'medium',
  ...
  if (submitted) return (
    <div className="p-6 flex flex-col items-center justify-center min-h-[60vh] text-center">
      ...
      <p className="text-[10px] text-slate-600 mt-3">4P3X Base is not a crisis service. If you need urgent help, please contact a professional.</p>
...
// ─── Activity Records Screen ─────────────────────────────────────
function ApplicationsScreen({ participant, dataStore, onBack }) {
  const [modal, setModal] = useState(null)
  const [form, setForm]   = useState({ organisation:'', roleTitle:'', source:'Indeed', dateApplied: new Date().toISOString().split('T')[0], status:'Applied', notes:'' })
  const set = (k,v) => setForm(f=>({...f,[k]:v}))
  const apps = dataStore.applications.filter(a => a.jobseekerId === participant.id && !a.isDemo)

  const handleSave = () => {
    dataStore.addApplication({ ...form, jobseekerId: participant.id })
    setModal(null)
    setForm({ organisation:'', roleTitle:'', source:'Indeed', dateApplied: new Date().toISOString().split('T')[0], status:'Applied', notes:'' })
  }

  return (
    <div className="p-4 pb-24 space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="font-semibold text-white">My Activity Records</h2>
        ...
        <div key={a.id} className="bg-[#0d1426] border border-slate-800/60 rounded-xl p-4">
          <div className="flex justify-between items-start gap-2">
            <div>
              ...
              <div className="text-xs text-slate-500">{a.organisation} · {a.source}</div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 backdrop-blur-sm">
          <div className="w-full bg-[#0d1426] border border-slate-800/60 rounded-t-2xl p-5 space-y-4 max-h-[85vh] overflow-y-auto">
            <div className="flex justify-between items-center">
              <span className="font-semibold text-white">Add Activity Record</span>
              <button onClick={() => setModal(null)} className="text-slate-500"><Icon name="X" size={18}/></button>
            </div>
            {[['organisation','Organisation'],['roleTitle','Role Title']].map(([k,l]) => (
              <div key={k}>
            ...
            <button onClick={handleSave} disabled={!form.organisation||!form.roleTitle}
...
// ─── Progress Screen ─────────────────────────────────────────
function ProgressScreen({ participant, metrics, weeklyTarget }) {
  return (
    <div className="p-4 pb-24 space-y-5">
      ...
      <div className="bg-[#0d1426] border border-slate-800/60 rounded-2xl p-5 flex flex-col items-center gap-4">
        <ProgressRing pct={metrics.weeklyTargetPercent} size={100} label={`of ${participant.weeklyTargetHours||weeklyTarget}h weekly target`}/>
        <div className="w-full space-y-2">
          {[
            ['Hours this week', `${metrics.weeklyHoursLogged}h`,'text-[#d4af37]'],
            ['Target', `${participant.weeklyTargetHours||weeklyTarget}h`,'text-slate-400'],
            ['Activity Records this week', metrics.applicationCountWeek,'text-purple-400'],
            ['Review Sessions upcoming', metrics.interviewsUpcoming,'text-cyan-400'],
...
export default function JobseekerApp() {
  ...
  const metrics = participant
    ? deriveJobseekerMetrics(participant.id, snap, participant.weeklyTargetHours || weeklyTarget)
    : null
  ...
  const renderTab = () => {
    switch(tab) {
      ...
      default:             return (
        <HomeScreen participant={participant} metrics={metrics} weeklyTarget={weeklyTarget}
  ...
  return (
    <div className="min-h-screen bg-[#050810] text-white pb-20">
      ...
      <div className="sticky top-0 z-40 flex items-center justify-between px-4 py-3 bg-[#090e1c]/95 backdrop-blur-md border-b border-slate-800/60">
        <div>
          <span className="font-display font-bold text-[#d4af37] text-sm">4P3X Base</span>
          <span className="text-[10px] text-slate-600 ml-2">Participant App</span>
        </div>
