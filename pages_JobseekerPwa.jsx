 ...
 *
 * Resolves a participant from their unique PWA link token.
...
const ACTIVITY_TYPES = [
  'Activity tracking','Job activity record','Progress portfolio update','Cover letter',
  'Review Session preparation','Review Session attended','Organisation contact',
  'Training / course','Assigned mentor appointment','Portfolio update',
  'Networking','Other employment activity',
]

const APP_STATUSES = [
  'Drafting','Applied','Awaiting response','Review Session offered',
  'Review Session completed','Rejected','Offer received','Withdrawn',
]
...
const TABS = [
  ...
  { id: 'application', label: 'Activity Record', icon: 'Send' },
  { id: 'review session',   label: 'Review Session',   icon: 'CalendarCheck' },
...
export default function JobseekerPwa() {
  ...
  const [participant, setJobseeker] = useState(null)
    ...
    async function resolve() {
      ...
      if (isDemoMode) {
        ...
        try {
          ...
          if (!jsId) {
            // Fallback: maybe publicLinkId IS the participant id directly (legacy)
      ...
      // Fetch full participant record from Supabase
      try {
        const { data, error } = await getCLSupabaseClient()
          .from('participants').select('*').eq('id', jobseekerId).single()
  ...
  }, [participant, dataStore, isDemoMode])
  ...
  return (
    <div className="min-h-screen bg-[#050810] flex flex-col max-w-lg mx-auto">
      ...
      <div className="sticky top-0 z-10 bg-[#050810]/95 backdrop-blur border-b border-slate-800/60 px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="text-sm font-bold text-white">4P3X Base</div>
              <div className="text-[10px] text-slate-600">{participant?.displayName || '—'}</div>
            </div>
          </div>
      ...
      <div className="flex-1 overflow-auto px-4 py-4 pb-24">
        {tab === 'home'        && <HomeTab js={participant} metrics={metrics} />}
        {tab === 'log'         && <LogActivityTab js={participant} dataStore={dataStore} isDemoMode={isDemoMode} publicLinkId={publicLinkId} onResult={flashMsg} />}
        {tab === 'application' && <ApplicationTab js={participant} dataStore={dataStore} isDemoMode={isDemoMode} publicLinkId={publicLinkId} onResult={flashMsg} />}
        {tab === 'review session'   && <InterviewTab   js={participant} dataStore={dataStore} isDemoMode={isDemoMode} publicLinkId={publicLinkId} onResult={flashMsg} />}
        {tab === 'checkin'     && <CheckInTab      js={participant} dataStore={dataStore} isDemoMode={isDemoMode} publicLinkId={publicLinkId} onResult={flashMsg} />}
        {tab === 'evidence'    && <EvidenceTab     js={participant} dataStore={dataStore} isDemoMode={isDemoMode} publicLinkId={publicLinkId} onResult={flashMsg} />}
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl p-5" style={{ background: '#070d1a', border: `1px solid ${GOLD}18` }}>
        ...
        <div className="mt-2 text-[10px] text-slate-700 text-center">
          Supports tracking progress toward a weekly target weekly assigned activities requirement.
          Final compliance decisions remain the responsibility of the relevant coach, practitioner, or authority.
        </div>
      </div>
      ...
      <div className="rounded-xl px-4 py-3 text-[10px] text-slate-700"
        ...
        <strong className="text-slate-500">4P3X Intelligent AI</strong> provides guidance and organisation support only.
        It does not replace official guidance, legal advice, medical advice, benefits advice, or human practitioner review.
      </div>
    </div>
  )
}
...
function LogActivityTab({ js, dataStore, isDemoMode, publicLinkId, onResult }) {
  ...
  const [form, setForm] = useState({
    ...
    activityType: 'Assigned activity', description: '', notes: '',
    durationMinutes: 0,
  })
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setForm({ date: today, startTime: '', endTime: '', activityType: 'Assigned activity', description: '', notes: '', durationMinutes: 0 })
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-white font-bold text-base">Log Activity Tracking Activity</h2>
...
function ApplicationTab({ js, dataStore, isDemoMode, publicLinkId, onResult }) {
  const today = new Date().toISOString().split('T')[0]
  const [form, setForm] = useState({ organisation: '', roleTitle: '', source: '', dateApplied: today, status: 'Applied', notes: '' })
  const [saving, setSaving] = useState(false)
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }))

  async function handleSubmit() {
    if (!form.organisation || !form.roleTitle) { onResult('error', 'Organisation and role title are required.'); return }
    setSaving(true)
    try {
      const record = {
        jobseeker_id: js.id, pwa_link_id: publicLinkId,
        organisation: form.organisation, role_title: form.roleTitle,
      ...
      onResult('success', 'Activity Record logged.')
      setForm({ organisation: '', roleTitle: '', source: '', dateApplied: today, status: 'Applied', notes: '' })
    } finally { setSaving(false) }
  }

  return (
    <div className="space-y-4">
      <h2 className="text-white font-bold text-base">Log Job Activity Record</h2>
      <Field label="Organisation / Company" required><Input value={form.organisation} onChange={e => set('organisation', e.target.value)} placeholder="e.g. Horizon Ltd" /></Field>
      ...
      <Field label="Notes"><Textarea value={form.notes} onChange={e => set('notes', e.target.value)} placeholder="Any notes about this activity record…" rows={2} /></Field>
      <Btn label={saving ? 'Saving…' : 'Save Activity Record'} icon="Send" color={BLUE} onClick={handleSubmit} disabled={saving} full />
    </div>
  )
}
...
function InterviewTab({ js, dataStore, isDemoMode, publicLinkId, onResult }) {
  const [form, setForm] = useState({ organisation: '', roleTitle: '', dateTime: '', locationOrLink: '', preparationTasks: '', outcome: '', notes: '' })
  const [saving, setSaving] = useState(false)
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }))

  async function handleSubmit() {
    if (!form.organisation || !form.dateTime) { onResult('error', 'Organisation and date/time are required.'); return }
    setSaving(true)
    try {
      const record = {
        jobseeker_id: js.id, pwa_link_id: publicLinkId,
      ...
      onResult('success', 'Review Session logged.')
      setForm({ organisation: '', roleTitle: '', dateTime: '', locationOrLink: '', preparationTasks: '', outcome: '', notes: '' })
    } finally { setSaving(false) }
  }

  return (
    <div className="space-y-4">
      <h2 className="text-white font-bold text-base">Log Review Session</h2>
      <Field label="Organisation / Company" required><Input value={form.organisation} onChange={e => set('organisation', e.target.value)} placeholder="e.g. Horizon Ltd" /></Field>
      ...
      <Btn label={saving ? 'Saving…' : 'Save Review Session'} icon="CalendarCheck" color={GREEN} onClick={handleSubmit} disabled={saving} full />
    </div>
  )
}
...
// ─────────────────────────────────────────────────────────────
const CI_QUESTIONS = [
  'Did you complete assigned activities today?',
...
function CheckInTab({ js, dataStore, isDemoMode, publicLinkId, onResult }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Field label="Hours on activity tracking today">
...
function EvidenceTab({ js, dataStore, isDemoMode, publicLinkId, onResult }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Field label="Title / Description" required><Input value={form.title} onChange={e => set('title', e.target.value)} placeholder="e.g. Screenshot of activity record confirmation" /></Field>
  ...
  return (
    <div className="min-h-screen bg-[#050810] flex items-center justify-center">
      <div className="text-center">
        ...
        <div className="text-slate-500 text-sm">Loading your 4P3X Base…</div>
      </div>
    </div>
  )
}
      ...
      <div className="w-full max-w-sm text-center space-y-5">
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm leading-relaxed">
            This 4P3X Base PWA link is invalid, expired, or has not been set up yet.
          </p>
        </div>
        {!isSupabase && !isDemoMode && (
          <div className="rounded-xl px-4 py-3 text-xs text-left"
            ...
            Supabase is not configured. Ask your coach to check the 4P3X Base settings.
          </div>
        )}
        <div className="text-[10px] text-slate-700 font-mono">Link ID: {linkId || '(none)'}</div>
        <div className="text-[10px] text-slate-700">
          Please ask your assigned mentor or support contact for a valid link.
        </div>
      </div>
    </div>
  )
