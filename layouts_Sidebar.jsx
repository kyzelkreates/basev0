...
function ApexBaseLogo({ onClose }) {
  return (
    <div className="flex items-center justify-between px-4 py-4 border-b border-[#2a1f00]/60 flex-shrink-0">
      ...
      <button
        onClick={onClose}
        className="p-1.5 rounded-md text-slate-500 hover:text-white hover:bg-slate-800/60 transition-colors flex-shrink-0 ml-2"
...
export default function Sidebar() {
  ...
  return (
    <aside
      ...
      aria-hidden={!isOpen}
    >
      <ApexBaseLogo onClose={close} />

      <nav
        className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-none px-2 py-2"
        style={{ minWidth: 0 }}
      >
      ...
      <div className="px-3 pb-2 space-y-2 flex-shrink-0">
        ...
        >
          ...
          <span className="text-xs font-bold text-[#d4af37] flex-1 text-left truncate">Admin Dashboard</span>
        ...
        >
          ...
          <span className="text-xs font-medium text-slate-500 flex-1 text-left truncate">Invite Participant</span>
          <Icon name="ChevronRight" size={10} className="text-slate-700 group-hover:text-slate-500 transition-colors flex-shrink-0" />
        </button>
      </div>
