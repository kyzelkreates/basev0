    ...
    <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg border"
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[10px] text-slate-500 truncate">
          {iv.organisation || 'Organisation'} · {d.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}
        </div>
      </div>
...
export default function Dashboard() {
  ...
  // ── Participants — computed once on mount / demo-mode change ─
  const [participants, setJobseekers] = useState([])
  const [allMetrics, setAllMetrics] = useState({})
  const seededRef = useRef(false)

  useEffect(() => {
    // Load participants from service (already seeded by AppCore on mount)
    const js = isDemoMode
  ...
  const activeCount   = participants.filter(j => j.status === 'active').length
  const inactiveCount = participants.filter(j => j.status === 'inactive').length
  const atRiskCount   = participants.filter(j => j.riskLevel === RISK_LEVEL.HIGH || j.riskLevel === RISK_LEVEL.CRITICAL).length
  const criticalCount = participants.filter(j => j.riskLevel === RISK_LEVEL.CRITICAL).length
  ...
  const avgTargetPct   = total > 0
    ...
    : 0
  const onTarget       = participants.filter(j => (allMetrics[j.id]?.weeklyTargetPercent || 0) >= 80).length
  const belowTarget    = participants.filter(j => (allMetrics[j.id]?.weeklyTargetPercent || 0) < 30).length
  ...
  const evidenceGaps   = participants.filter(j =>
    j.status === 'active' && evidence.filter(e => e.jobseekerId === j.id).length === 0
  ).length

  // ── Sorted & sliced lists ───────────────────────────────────
  const jsMap       = participants.reduce((acc, j) => { acc[j.id] = j; return acc }, {})
  const riskOrder   = { critical: 0, high: 1, medium: 2, low: 3 }
  const byRisk      = [...participants].sort((a, b) => (riskOrder[a.riskLevel] ?? 4) - (riskOrder[b.riskLevel] ?? 4)).slice(0, 8)
  ...
  const byTarget    = [...participants].sort((a, b) => (allMetrics[a.id]?.weeklyTargetPercent || 0) - (allMetrics[b.id]?.weeklyTargetPercent || 0)).slice(0, 6)
  ...
  const feedItems = [
    ...actLogs.slice(0, 6).map(a => ({
      ...
      label: `${jsMap[a.jobseekerId]?.displayName || 'Participant'} logged ${a.activityType || 'activity'}`,
    ...
    ...apps.filter(a => inWeek(a.dateApplied || a.createdAt)).slice(0, 3).map(a => ({
      ...
      label: `${jsMap[a.jobseekerId]?.displayName || 'Participant'} applied — ${a.organisation || a.roleTitle || 'role'}`,
      sub: a.roleTitle || '',
    })),
    ...chks.filter(c => inWeek(c.date || c.createdAt)).slice(0, 3).map(c => ({
      icon: 'CheckCircle2', color: C.green, date: c.date || c.createdAt,
      label: `${jsMap[c.jobseekerId]?.displayName || 'Participant'} completed check-in`,
      sub: '',
    })),
  ...
  return (
    <div className="p-4 md:p-6 space-y-5 min-h-full">
          ...
          <div className="flex items-center gap-2.5 mb-1">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-600">
                {config?.organisationName || '4P3X Base Participant Management'} ·{' '}
      ...
      {/* ── ROW 1: PRIMARY KPIs ─────────────────────────── */}
      <div>
        <SectionTitle icon="BarChart3" label="Participant Overview" color={C.gold} />
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <KpiCard label="Total Participants"        value={total}           sub={`${activeCount} active · ${inactiveCount} inactive`}             icon="Users"          color={C.gold}   onClick={() => navigate('/participants')} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {/* Participant risk list */}
        <div className="rounded-xl p-4" style={{ background: '#070d1a', border: `1px solid ${C.gold}14` }}>
          <SectionTitle icon="Users" label="Participants by Risk" color={C.gold} onMore={() => navigate('/participants')} />
          <div className="space-y-0.5">
            {byRisk.length === 0
              ? <Empty icon="Users" text="No participants yet. Demo mode adds sample data." />
              : byRisk.map(js => (
                  <JsRow key={js.id} js={js} metrics={allMetrics[js.id]}
                    onClick={() => navigate(`/participants/${js.id}`)} />
                ))}
          </div>
        </div>
        ...
        <div className="rounded-xl p-4" style={{ background: '#070d1a', border: `1px solid ${C.purple}14` }}>
          ...
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg mb-3"
            ...
            <div>
              ...
              <div className="text-[10px] text-slate-600">{totalHrsWeek}h logged · 35h target per participant</div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 rounded-xl p-4" style={{ background: '#070d1a', border: `1px solid ${C.silver}10` }}>
          ...
          {feedItems.length === 0
            ? <Empty icon="Activity" text="Activity will appear as participants log their work." />
