...
const STATUSES = ['Drafting','Applied','Awaiting response','Review Session offered','Review Session completed','Rejected','Offer received','Withdrawn']
const SOURCES  = ['Indeed','Reed','LinkedIn','Total Jobs','Jobsite','Company website','Referral','Recruitment agency','Assigned mentor','Other']
...
const STATUS_COLORS = {
  ...
  'Review Session offered':   'text-purple-400  border-purple-700',
  'Review Session completed': 'text-cyan-400    border-cyan-700',
...
function AppModal({ existing, participants, onClose, onSaved }) {
  const dataStore = useDataStore()
  const [form, setForm] = useState({
    jobseekerId: '', organisation: '', roleTitle: '', source: 'Indeed',
  ...
  const handleSave = () => {
    if (!form.jobseekerId || !form.organisation || !form.roleTitle) return
    setSaving(true)
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-lg bg-[#0d1426] border border-slate-800/60 rounded-2xl overflow-hidden animate-slide-up">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800/40">
          <span className="font-semibold text-white">{existing?.id ? 'Edit Activity Record' : 'Add Activity Record'}</span>
          <button onClick={onClose} className="text-slate-500 hover:text-white"><Icon name="X" size={18} /></button>
        </div>
        <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
          <div>
            <label className="text-xs text-slate-400 font-medium mb-1 block">Participant <span className="text-red-400">*</span></label>
            <select value={form.jobseekerId} onChange={e => set('jobseekerId', e.target.value)}
              ...
              {participants.map(js => <option key={js.id} value={js.id}>{js.displayName}{js.isDemo ? ' [DEMO]' : ''}</option>)}
            </select>
          </div>
          {[['organisation','Organisation / Company',true],['roleTitle','Role Title',true]].map(([k, label, req]) => (
            <div key={k}>
        ...
        <div className="px-5 py-4 border-t border-slate-800/40 flex gap-2">
          ...
          <button onClick={handleSave} disabled={saving || !form.jobseekerId || !form.organisation || !form.roleTitle}
            ...
            {saving ? 'Saving…' : 'Save Activity Record'}
          </button>
        </div>
      </div>
    </div>
...
export default function ActivityRecords() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-800/60">
                  {['Participant','Organisation','Role','Source','Date','Status','Notes'].map(h => (
              ...
              <tbody>
                {filtered.map(a => (
                  <tr key={a.id} onClick={() => setModal(a)}
                    ...
                    <td className="px-4 py-3 text-xs text-white">{a.organisation}</td>
      ...
      {modal !== null && (
        <AppModal participants={participants} existing={modal} onClose={() => setModal(null)} onSaved={() => {}} />
      )}
    </div>
  )
}
