  ...
  return (
    <div className={`rounded-xl border p-4 ${riskColor} space-y-2`}>
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-sm font-semibold text-white">{insight.participantName}</div>
      ...
      {insight.suggestedAction && (
        <div className="flex items-start gap-2 px-3 py-2 rounded-lg bg-slate-900/50 border border-slate-800/40">
          ...
          <p className="text-[10px] text-slate-300 leading-relaxed">
            <strong className="text-purple-300">Suggested action:</strong> {insight.suggestedAction}
          </p>
        </div>
      )}
...
function generateAutoInsights(participants, dataStore, isDemoMode, weeklyTarget) {
  const now    = new Date()
  ...
  participants.forEach(js => {
    ...
    if (metrics.checkInsThisWeek === 0) {
      ...
      suggestedAction = suggestedAction || 'Prompt participant to complete their daily check-in.'
    }
    ...
    if (riskLevel !== 'low') {
      insights.push({
        jobseekerId: js.id,
        participantName: js.displayName,
        programme: js.programme || 'Participant Management',
        riskLevel,
    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-widest">4P3X Intelligent AI 1 — Admin Guide</span>
        </div>
        <AssistantPanel
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-widest">4P3X Intelligent AI 2 — Progress Insight</span>
        </div>
        ...
        {mode !== AI_MODES.OFF && insights.length > 0 && (
          <div className="mb-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] text-slate-600">{insights.length} flagged · {participants.length} participants</span>
            </div>
            {insights.map(i => <AutoInsightCard key={i.jobseekerId} insight={i} />)}
          </div>
        )}

        {mode !== AI_MODES.OFF && insights.length === 0 && participants.length > 0 && (
          <div className="mb-4 py-6 text-center bg-[#0d1426] border border-emerald-700/20 rounded-xl">
            ...
            <p className="text-sm text-slate-400 font-medium">All participants on track</p>
        ...
        <AssistantPanel
          ...
          initialMessage={participants.length > 0 ? handleAi2Query('action_summary', ai2Context) : null}
          aiConfig={aiConfig}
        />
      </div>
