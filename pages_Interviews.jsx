/**
 * ============================================================
 * 4P3X Learning & Monitoring Base OS™ — Review Sessions Page
...
function InterviewModal({ existing, participants, onClose, onSaved }) {
  const dataStore = useDataStore()
  const [form, setForm] = useState({
    jobseekerId:'', organisation:'', roleTitle:'',
  ...
  const handleSave = () => {
    if (!form.jobseekerId || !form.organisation) return
    setSaving(true)
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-lg bg-[#0d1426] border border-slate-800/60 rounded-2xl overflow-hidden animate-slide-up">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800/40">
          <span className="font-semibold text-white">{existing?.id ? 'Edit Review Session' : 'Add Review Session'}</span>
        ...
        <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          {[['organisation','Organisation / Company',true],['roleTitle','Role Title',false]].map(([k,label,req]) => (
            <div key={k}>
        ...
        <div className="px-5 py-4 border-t border-slate-800/40 flex gap-2">
          ...
          <button onClick={handleSave} disabled={saving || !form.jobseekerId || !form.organisation}
        ...
        <div key={iv.id} onClick={() => setModal(iv)}
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="min-w-0">
              <div className="text-xs font-semibold text-white truncate">{iv.roleTitle || iv.organisation}</div>
              <div className="text-[10px] text-slate-500">{iv.organisation} · {jsMap[iv.jobseekerId]?.displayName || '—'}</div>
            </div>
  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="font-display font-bold text-white text-xl">Review Sessions</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Section title="Upcoming Review Sessions" items={upcoming} icon="CalendarCheck" color="text-purple-400"/>
        <Section title="Past Review Sessions" items={past} icon="History" color="text-slate-400"/>
      </div>
      {modal !== null && <InterviewModal participants={participants} existing={modal} onClose={() => setModal(null)} onSaved={() => {}}/>}
    </div>
  )
}
